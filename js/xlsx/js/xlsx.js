...
function xlsx(file) { 
	...
	var result,  zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, index, data, val, style, borders, border, borderIndex, fonts, font, fontIndex,
   ...
   if (typeof file === 'string') { 
   	// Load
		zipTime = Date.now();
		zip_new = zip_new.load(file, { base64: true });
		result = { worksheets: [], zipTime: Date.now() - zipTime };
		processTime = Date.now();

		// Process sharedStrings
		sharedStrings = []; s = zip_new.file('xl/sharedStrings.xml');
		if (s) {
		...
		s = zip_new.file('docProps/core.xml').asText();
		...
		s = zip_new.file('xl/workbook.xml').asText(); index = s.indexOf('activeTab="');
		if (index > 0) {
			s = s.substr(index + 11); 
		...
		styles = [];
		s = zip_new.file('xl/styles.xml').asText().split('<numFmt '); i = s.length;
		while (--i) { 
		...
		while (i--) {
			s = zip_new.file('xl/worksheets/sheet' + (i + 1) + '.xml' ).asText().split('<row ');
		...
		sharedStrings = [[], 0];
		// Fully static
		zip_new.folder('_rels').file('.rels', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/extended-properties" Target="docProps/app.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties" Target="docProps/core.xml"/><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>');
		docProps = zip_new.folder('docProps');

		xl = zip_new.folder('xl');
    ...
    fonts = new Array(1);
		...
		while (w--) { 
			...
			worksheet = file.worksheets[w]; data = worksheet; //.data; modified by lakshmi
			s = '';
			columns = [];
    	merges = [];
			i = -1; l = data.length;
		...
		// [Content_Types].xml
		zip_new.file('[Content_Types].xml', '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/>'
		...
		zipTime = Date.now();
		result = {
			base64: zip_new.generate({ compression: 'STORE' }), zipTime: Date.now() - zipTime, processTime: processTime,
